...
import {
  deleteInvoice,
  getAllInvoice,
} from "../../../features/invoice/invoiceSlide";
import {
  deleteInvoiceDetail,
  getAllInvoiceDetail,
...
function CancelOrder() {
  ...
  const uploadCart = (products) => {
    ...
    products.forEach(product => {
      ...
      dispatch(deleteInvoice(product.invoiceId));
      dispatch(deleteInvoiceDetail(product.invoiceDetailId));
    });
  };
        ...
        <div className="container-account">
          ...
          <div className="container" style={{"backgroundColor":"#f8f8f8"}}>
              ...
              {groupedProductsArray
                .filter((group) =>
                  group.products.some(
                    (product) => product.paymentStatus === "Đã Hủy"
                  )
                )
                .map((group) => (
                            ...
                            <button
                              onClick={() =>
                                uploadCart({
                                  ...
                                  pricenew: product.unitPrice,
                                  quantity: product.quantity,
                                  invoiceId: product.invoice.id,
                                  invoiceDetailId: product.id,
                                })
                              }
                        ...
                        {group.products.length > 1 &&
                          index === group.products.length - 1 && ( // Chỉ hiển thị header cho sản phẩm đầu tiên trong nhóm
                            <div className="body-footer">
                              ...
                              <button
                                 onClick={() =>
                                  uploadCart(group.products.map(product => ({
                                    productId: product.product.id,
                                    pricenew: product.price-product.price*(product.productSale.percentDiscount/100),
                                    quantity: product.quantity,
                                    invoiceId: product.invoice.id,
                                    invoiceDetailId: product.id,
                                  })))}
