  ...
  return (
      ...
      <div className="Inner">
          ...
          <div className="header-order">
            <nav>
              ...
              <Link to="/account/orderdetail/confim">
                  ...
                  <Link to="/account/orderdetail/deliver"><li className="active">Chờ Giao Hàng</li></Link>
                  <Link to="/account/orderdetail/driversuccess">
                      <li className="">Giao Thành Công</li>
                    </Link>
                  <Link to="/account/orderdetail/successorder"><li>Hoàn Thành</li></Link>
                  <Link to="/account/orderdetail/cancel"><li>Đã Hủy</li></Link>
              </ul>
            </nav>
          </div>
          {groupedProductsArray
                .filter((group) =>
                  group.products.some(
                    (product) => product.paymentStatus === "Chờ Giao Hàng"
                  )
                )
                .map((group) => (
              ...
              {group.products.map((product, index) => (
                <div key={index} className="container-body">
                  ...
                  {group.products.length > 1 && index === group.products.length - 1 && ( // Chỉ hiển thị header cho sản phẩm đầu tiên trong nhóm
                      <div className="body-footer">
                    <div className="body-footer--sum">
                      <span>Thành tiền:</span>
                      <h4>
                {formatPrice(
                  group.products.reduce((total, prod) => (
                    total + (prod.price - prod.price * (prod.productSale.percentDiscount / 100))
                  ), 0)
                )}
              </h4>
                    </div>
